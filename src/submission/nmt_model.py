...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for t in range(target_padded.size(0)):
            
            Y_t_embed = self.model_embeddings.target(target_padded[t])
            Ybar_t = torch.cat((Y_t_embed, o_prev), dim=-1)
            
            dec_state, combined_output, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_masks)
